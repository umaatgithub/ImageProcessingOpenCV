...
EdgeDetectionImage::EdgeDetectionImage(QObject *parent) : QObject(parent),
    qtOpenCVBridge(new QtOpenCVBridge)
{

}

QImage EdgeDetectionImage::applyCannysEdgeDetector(const QImage &inputImage, int radius)
{
...
QImage EdgeDetectionImage::applySobelEdgeDetector(const QImage &inputImage, int radius)
{
    cv::Mat inputMat = qtOpenCVBridge->QImage2Mat(inputImage);
    cv::Mat outputMat ;
    if(inputMat.type() == CV_8UC4 || inputMat.type() == CV_8UC3){
...
QImage EdgeDetectionImage::applyScharrEdgeDetector(const QImage &inputImage, int radius)
{
    cv::Mat inputMat = qtOpenCVBridge->QImage2Mat(inputImage);
    cv::Mat outputMat ;
    if(inputMat.type() == CV_8UC4 || inputMat.type() == CV_8UC3){
