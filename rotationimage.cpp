#include "rotationimage.h"

RotationImage::RotationImage(QObject *parent) : QObject(parent),
    qtOpenCVBridge(new QtOpenCVBridge)
{

}

QImage RotationImage::applyAntiClockwiseRotation(const QImage &inputImage, int radius)
{
    cv::Mat inputMat = qtOpenCVBridge->QImage2Mat(inputImage);
    cv::Mat outputMat ;
    Point2f pt(inputMat.cols/2., inputMat.rows/2.);
    Mat r = getRotationMatrix2D(pt, radius, 1.0);
    warpAffine(inputMat, outputMat, r, Size(inputMat.cols, inputMat.rows));
    return qtOpenCVBridge->Mat2QImage(outputMat);
}

QImage RotationImage::applyClockwiseRotation(const QImage &inputImage, int radius)
{
    cv::Mat inputMat = qtOpenCVBridge->QImage2Mat(inputImage);
    cv::Mat outputMat ;
    Point2f pt(inputMat.cols/2., inputMat.rows/2.);
    Mat r = getRotationMatrix2D(pt, 360-radius, 1.0);
    warpAffine(inputMat, outputMat, r, Size(inputMat.cols, inputMat.rows));
    return qtOpenCVBridge->Mat2QImage(outputMat);
}
